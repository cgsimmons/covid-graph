...
const { DEFAULT_ARGS } = require('./consts');
...
param
    .option(
        '-d, --days <number>',
        'number of days the chart will go back',
        myParseInt,
        DEFAULT_ARGS.days,
    )
    .option(
        '-c, --country <name>',
        'target country (country name, iso2, iso3, or country ID code)',
        String,
        DEFAULT_ARGS.country,
    )
    .option(
        '-h, --height <number>',
        'max terminal chart height',
        myParseInt,
        DEFAULT_ARGS.height,
    )
